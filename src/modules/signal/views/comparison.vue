  ...
  if (promiseList && promiseList.length > 0) {
    ...
    Promise.all(promiseList).then((res = []) => {
      ...
      for (const detailItem of props.detailDataList) {
        ...
        const data = getDataListByAttId(<[]>res, detailItem?.attachmentId)
        options.series.push({
          name: detailItem?.fileCode,
          symbolSize: 5,
          data: parseAllData(data),
          type: 'line',
          smooth:true,
          showSymbol:false,
          barGap: 0
...
function getDataListByAttId(arr: [], attId: any) {
  console.log('arr, attId===>', arr, attId)
  const findItem = arr.find((item) => { return item.id == attId })
...
<template>
  <el-scrollbar :always="true" :native="true" :min-size="40"
    ...
    <div :style="{height: `${(100 + 500 + detailDataList.length * 400)}px`}">
      <el-tabs type="border-card" style="margin: 10px;">
        <el-tab-pane v-for="(detailData, index) in detailDataList" :key="index" :label="detailData.fileCode">
          <el-descriptions
            ...
            style="margin: 20px;"
          >
            <!-- width="120px" class-name="descriptions-item" -->
            <el-descriptions-item v-for="(item, idx) in detailProps" :key="idx">
              ...
              <div>
                ...
                <span v-else>
                         ...
                         @click="downloadCsv(detailData)">点击下载：{{ detailData['attachmentName'] }}</el-link>
      ...
      <div v-for="(item, index) in detailDataList" v-show="item.showChart" :id="`id_${item.attachmentId}`" :key="index" v-loading="chartLoadings" class="chart-main"/>
      <br><br>
    </div>
  </el-scrollbar>
</template>
